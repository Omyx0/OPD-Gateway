...
export default function DashboardLayout() {
  ...
  const isWizardRoute = 
    location.pathname === '/dashboard/symptoms' || 
    location.pathname === '/dashboard/triage';
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-[#e8f1f7] flex justify-center items-center antialiased">
      ...
      <div className="min-h-[100dvh] w-full max-w-md bg-[#eef4f8] shadow-2xl flex flex-col relative overflow-hidden isolate">
        ...
        {/* Top App Header (Only on main dashboard tabs) */}
        {!isWizardRoute && (
        ...
        {/* Main Content Area */}
        <main className={`flex-1 overflow-y-auto relative z-10 ${isWizardRoute ? '' : 'pb-24'}`}>
          <Outlet />
        </main>

        {/* Floating Bottom Navigation Bar (Hidden during Intake/Triage wizard) */}
        {!isWizardRoute && (
